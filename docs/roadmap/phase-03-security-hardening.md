...
| 03.4 | Frontend CSP + HSTS | `next.config.ts` ships full headers: HSTS preload, Permissions-Policy, CSP with `frame-ancestors 'none'` and `img-src`/`connect-src` computed from `NEXT_PUBLIC_API_URL` at build time. | araguaney `next.config.ts` | 🟡 | ✅ Done |
