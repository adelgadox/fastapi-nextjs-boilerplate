...
| 04.4 | Wire Sentry properly | Backend + worker (separate process init) + frontend three-file wiring (`instrumentation.ts`, `instrumentation-client.ts` — Sentry 10 filename gotcha —, server/edge configs), Session Replay on errors, `/monitoring` tunnel route, env cascade (`sentry-environment.ts`), fail-loud build guard on half-configured source maps. | araguaney | 🟢 | ✅ Done |
