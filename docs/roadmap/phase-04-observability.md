...
| 04.6 | Slack alerting + infra status | `#backend-alerts` on 500s / DB errors, enriched with Railway/Vercel incident status. Boilerplate has `utils/slack.py` + `infra_status.py` already — wire into the 500 handler. | both `utils/slack.py`, `infra_status.py` | 🟢 | 🟡 Partial |
...
| 04.9 | Uptime monitoring | UptimeRobot on frontend + `/health` at 5-min intervals. | both | 🟢 | ⬜ Pending |
